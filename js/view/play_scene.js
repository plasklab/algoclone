...
var ExecBlock = enchant.Class.create(enchant.Sprite, {
    initialize: function(scene, imgsrc) {
...
var FunctionFrameView = enchant.Class.create({
    ...
    displayBlocks: function(tokens) {
        ...
        for (var i = 0; i < tokens.length; i++) {
            var token = tokens[i];
            token.ui.showAt(headX, headY + i * 32);
            this.blocks.push(token.ui);
        }
    },
    delete: function() {
...
var PlayScene = enchant.Class.create(enchant.Scene, {
    initialize: function(mapId, program) {
        ...
        var playScene = this;
        var highlightBlock = undefined;
        var engine = new Interp(program, {
            highlight: function(token) {
                console.log("highlight: "+token.type);
                if (highlightBlock)
                    highlightBlock.highlight(false);
                highlightBlock = token.ui;
                highlightBlock.highlight(true);
            },
            forward: function() {
                var nextX, nextY;
		...
		if (frame.type == "FunctionFrame") {
            ...
            initToken: function(token, i) {
                ...
                var b = new ExecBlock(playScene, imgid);
                token.ui = b;
            }
        });
        this.engine = engine;
